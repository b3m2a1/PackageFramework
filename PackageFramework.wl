...
PackageContext::usage="PackageContext[pkg] returns the main context for the package";
...
PackageAddLoadedPackages::usage="";
...
ConstructPackageFrameworkPackage[
  ...
  With[{loc=pkg["Location"]},
    If[StringQ@loc&&DirectoryQ@loc,
      Replace[
        ResolvePackageFrameworkPackage[loc, parent],
        None:>
          ConstructPackageFrameworkPackage[
...
ConstructPackageFrameworkPackage[
  ...
  With[{loc=OptionValue["Location"]},
    If[StringQ[loc]&&DirectoryQ[loc],
      Replace[
        ResolvePackageFrameworkPackage[loc, None],
        None:>
          ConstructPackageFrameworkPackage[
            PackageFrameworkConfig[{ops}, None, loc],
            None,
...
ConstructPackageFrameworkPackage[
  a_Association, 
  parent:_PackageFrameworkPackage|None,
  validate_:True
  ]:=
  If[validate,
    With[{loc=a["Location"], cont=First@Flatten@{a["Context"]}},
      If[StringQ[loc]&&DirectoryQ[loc],
        Replace[
          ...
          None:>
            Replace[
              ResolvePackageFrameworkPackage[cont, parent],
              None:>
                With[{config=PackageFrameworkConfig[Normal[a], None, loc]},
                  ConstructPackageFrameworkPackage[a, parent, False]
                  ]
              ]
          ],
        $Failed
...
PackageContext[pkg_PackageFrameworkPackage, Optional[Automatic, Automatic]]:=
  PackageContext[pkg, 
    Replace[PropertyValue[pkg, "ParentContext"],
      Except[_String]->""
      ]
    ]
...
AddPackageSymbols[pkg_, syms_]:=
  SetProperty[pkg, 
    "Symbols"->
      DeleteDuplicates@Flatten[{PackageSymbols[pkg], syms}]
    ]
  ...
  Block[
    {
      ...
      notIgnored=With[{ss=pkg["AutoloadIgnored"]}, nonProtectedSym[ss]]
      },
    If[!MemberQ[$loadingChain[pkg], pkgFile],
      (* all of this is to break any loops *)
      $inLoad[pkg]=True;
      AppendTo[$loadingChain[pkg], pkgFile];
      
      (* Clear, but only for OwnValues *)
      Replace[syms,
        (s_Symbol?notIgnored)|Verbatim[HoldPattern][s_Symbol?notIgnored]:>
          (OwnValues[s]={}),
        1
        ];
      
      If[!MemberQ[PackageLoadedPackages[pkg], pkgFile],
        PackageAddLoadedPackages[pkg, pkgFile];
        Internal`WithLocalSettings[
